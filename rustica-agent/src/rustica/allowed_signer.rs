...
impl RusticaServer {
    pub async fn get_allowed_signers_async(&self) -> Result<String, RefreshError> {
        ...
        let mut allowed_signers_decoder =
            match zstd::stream::Decoder::new(response.compressed_allowed_signers.as_bytes()) {
                Ok(decoder) => decoder,
                Err(e) => {
    ...
    pub fn get_allowed_signers(&self, handle: &Handle) -> Result<String, RefreshError> {
        handle.block_on(async { self.get_allowed_signers_async().await })
    }
}
