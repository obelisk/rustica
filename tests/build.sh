...
./target/debug/rustica --config examples/rustica_local_file_alt.toml > /dev/null 2>&1 &
RUSTICA_PID=$!
sleep 2
...
if ./target/debug/rustica-agent --config examples/rustica_agent_local.toml -i -o /tmp/testing_cert > /dev/null 2>&1; then
