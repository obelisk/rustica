...
pub fn verify_piv_certificate_chain(client: &[u8], intermediate: &[u8]) -> Result<Key, RusticaServerError> {
    ...
    let pin_policy = PinPolicy::try_from(validated_piv_data.pin_policy).map_err(|_| RusticaServerError::InvalidKey)?;
    let touch_policy = TouchPolicy::try_from(validated_piv_data.touch_policy).map_err(|_| RusticaServerError::InvalidKey)?;
